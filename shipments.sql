...
-- calculate the total selling price and total profits for all orders
select 
...
-- write a query to find all orders from the 'Technology' from category, that were shipped using 'second class' ship mode, ordered by order date
select
...
-- find the city and ship mode with the highest total quantity of products ordered
...
-- use a widow function to rank orders in each region by quantity in desc order
...
-- find the month of each order_id
select Order_Id, Order_Date, month(Order_Date) as Month from orders;

-- list all the orders placed in the first quarter of any year (Jan to Mar), including the total cost for these orders.
...
-- aternate using window function
...
-- find top 10 highest profit generating products using dense_rank
select Product_Id, cast(sum(Total_Profit) as decimal(10, 2)) as Highest_Profits, 
...
-- -- find top 10 highest revenue generating products by rankings
select Product_ID,
sum(Quantity * Unit_Selling_Price) as Total_Revenue,
...
-- find top 10 highest total sales city and ship mode of generating products
...
-- find the products in each region of total profit with rankings based on region
with cte as(
...
-- month over month growth comparision and percentage growth between 2022 and 2023
with cte as(
...
-- month over month as (jan, feb, ..) growth comparision and percentage growth between 2022 and 2023
with cte as(
